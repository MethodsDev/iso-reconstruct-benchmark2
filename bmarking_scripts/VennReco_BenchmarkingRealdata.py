...
UTILDIR = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "../workflows/lr_isoform_custom_docker")
)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="transcripts reconstructed by methods",
        nargs="+",
    )
    ...
    cmd = f"gffcompare -o {dataset_name}.denovo {ref_gtf} " + " ".join(reco_gtfs)
    logger.info(cmd)
    ...
    cmd = " ".join(
        [
            ...
            "--tracking " + f"{dataset_name}.denovo.tracking",
    ...
    cmd = " ".join(
        [
            "python3",
            os.path.join(UTILDIR, "summarize_denovo_analysis.TP_FP_FN.py"),
            "--tracking " + f"{dataset_name}.denovo.tracking",
            "--dataset " + dataset_name,
            "--ordered_tools " + " ".join(tool_names),
        ]
    )
    logger.info(cmd)
